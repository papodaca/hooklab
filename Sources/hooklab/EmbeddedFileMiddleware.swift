...
struct EmbeddedFileMiddleware: AsyncMiddleware {
    func respond(to request: Request, chainingTo next: any AsyncResponder) async throws -> Response {
        ...
        if let fileBytes = EmbeddedAssets.files[path] {
            return self.response(for: fileBytes, with: request, fileExtension: (path as NSString).pathExtension)
        }
    ...
    private func response(for bytes: [UInt8], with request: Request, fileExtension: String) -> Response {
